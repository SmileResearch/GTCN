...
def CNewMake_task_input(batch_data, get_num_edge_types=False):
...
class CNewGraphDataset(Dataset):
    def __init__(self, root, value_dict, type_dict, graph_node_max_num_chars=19, max_node_per_graph=50,max_graph=20000, max_variable_candidates=5, transform=None, pre_transform=None):
        ...
        super(CNewGraphDataset, self).__init__(root, transform, pre_transform)
    ...
    def processed_dir(self):
        ...
        return osp.join(self.root, "processed_new_c")
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            with open(raw_path, "r") as f:
                ...
                dfgEdges = raw_sample["ddgEdges"]
                ncsEdges = raw_sample["ncsEdges"]
                candidateNodeListRaw = raw_sample["candidateNodeList"]
                candidateNodeList = [node for node in candidateNodeListRaw if node < self.max_node_per_graph]
                #three_neighbors_adj = get_neighbors(astEdges, 3)
                #five_neighbors_adj = get_neighbors(astEdges, 5)

                if not astEdges or not cfgEdges or not dfgEdges or not ncsEdges:
                    continue
                ...
                candidate_name = [nodeDict[str(node)] for node in candidateNodeList]
                candidate_id = [node for node in candidateNodeList]
                # nop candidate 
...
if __name__ == '__main__':
    ...
    gd = CNewGraphDataset("data/train_data", value_dict=vocab_dict)
